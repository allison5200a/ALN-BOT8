...
* REFAZER OF SRALLISON × ALN GAY:V
...
const { help } = require('./ALN/help')
const { rules } = require('./ALN/rules')
const { bahasa } = require('./ALN/bahasa')
const { donasi } = require('./ALN/donasi')
const { limits } = require('./ALN/limit')
const { bokep } = require('./ALN/bokep.js')
const { ownermenu } = require('./ALN/ownermenu.js')
const { groupmenu } = require('./ALN/groupmenu.js')
const { mediamenu } = require('./ALN/mediamenu.js')
const { nsfwmenu } = require('./ALN/nsfwmenu.js')
const { listmenu } = require('./ALN/listmenu.js')
const { funmenu } = require('./ALN/funmenu.js')
const { makermenu } = require('./ALN/makermenu.js')
//jangan hiraukan ini!!,,,ini uji coba yang gagal:) kontol//
//const { gabutmenu } = require('./ALN/gabutmenu')
//const { ownermenu } = require('./ALN/ownermenu')
//const { groupmenu } = require('./ALN/groupmenu')
//const { vipmenu } = require('./ALN/vipmenu')
//const { nsfwmenu } = require('./ALN/nsfwmenu')
//const { creatormenu } = require('./ALN/creatormenu')
//const { downloadmenu } = require('./ALN/downloadmenu')
//const { allmenu } = require('./ALN/allmenu')
...
async function starts() {
	...
	client.on('qr', () => {
		console.log(color('[','white'), color('!','red'), color(']','white'), color('HUNT SCAN !! Assine ALN BOT'))
	})
	...
	client.on('chat-update', async (mek) => {
		try {
			...
			const ALNBOTS = ["558688036194@s.whatsapp.net"]
     ...
     const bayarLimit = (sender, amount) => {
		...
		case 'daftar':
			...
			case 'fadhil':
                ...
                setTimeout( () => {
					costum(help(prefix, instagram, yt, name, pushname2, user, limitt), text, ALNBOTS, replySet)
					}, 1500)
					setTimeout( () => {
           ...
           if (!isUser) return reply(mess.only.userB)
            ...
            break
				...
				if (!isUser) return reply(mess.only.userB)
					...
					client.sendMessage(from, 'Pergunta : Verificar Handsome Bang *'+ganteng+'*\n\nResposta : '+ teng +'', text, { quoted: mek })
					break
				case 'cantikcek':
				case 'cekcantik':
				if (isBanned) return reply(mess.only.benned)    
...
*/
              case 'anime':
				...
				if (!isUser) return reply(mess.only.userB)
				if (isLimit(sender)) return reply(limits.limitend(pushname2))
					if (!isAnime) return reply(' *Deve ativar o modo Animee* ')
					reply(mess.wait)
		    ...
		    case 'map':
				...
				case 'bc':
					...
					if (isMedia && !mek.message.videoMessage || isQuotedImage) {
						...
						for (let _ of anu) {
							sendMess(_.jid, `[ *ANLBOT BROADCAST* ]\n\n${body.slice(4)}`)
						}
						reply('Transmissão de sucesso')
					}
					break
		...
		if (isLimit(sender)) return reply(limits.limitend(pushname2))
		 ...
		 case 'tomp3':
				...
				if (!isUser) return reply(mess.only.userB)
					...
					exec(`ffmpeg -i ${media} ${ran}`, (err) => {
						fs.unlinkSync(media)
						if (err) return reply('Falha ao converter vídeo para mp33')
...
// akhir encrypt & decrypt Fitur
			...
			case 'google':
               ...
               if (isBanned) return reply(mess.only.benned)    
				if (!isUser) return reply(mess.only.userB)
				if (isLimit(sender)) return reply(limits.limitend(pushname2))
                if(googleQuery == undefined || googleQuery == ' ') return reply(`*Resultado da pesquisa : ${googleQuery}* não encontradon`)
